...
async function remove(id: string) {
  if (!await confirm({ message: 'Ο κωδικός θα διαγραφεί οριστικά.' })) return
...
<template>
  <div>
    <div class="flex items-center justify-between mb-5">
      <div>
        <h2 class="text-xl md:text-2xl font-bold text-gray-900">Κωδικοί</h2>
        <p class="text-sm text-gray-400 mt-0.5">Αποθηκευμένα διαπιστευτήρια παρόχων</p>
      </div>
      <button
        @click="openAdd"
    ...
    <div v-if="store.credentials.length === 0"
      ...
      <p class="text-gray-600 font-medium">Δεν έχεις αποθηκεύσει κωδικούς ακόμα</p>
      <p class="text-sm text-gray-400 mt-1">Πρόσθεσε τα credentials των παρόχων σου για γρήγορη πρόσβαση</p>
    ...
    <div v-else class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
      <div
        ...
        :key="cred.id"
        class="bg-white rounded-xl border border-gray-200 p-4 shadow-sm flex flex-col gap-3"
      >
        <!-- Header -->
        <div class="flex items-start justify-between gap-2">
          <div class="min-w-0">
            <p class="font-semibold text-gray-900 truncate">{{ cred.label }}</p>
        ...
        <!-- Username row -->
        <div v-if="cred.username" class="flex items-center gap-2 bg-gray-50 rounded-lg px-3 py-2">
          <span class="text-xs text-gray-400 w-16 shrink-0">Χρήστης</span>
          <span class="flex-1 text-sm text-gray-800 font-mono truncate">{{ cred.username }}</span>
          <button
          ...
          >
            {{ copiedKey === cred.id + '-u' ? '✓ Copied' : 'Copy' }}
          </button>
        </div>

        <!-- Password row -->
        <div v-if="cred.password" class="flex items-center gap-2 bg-gray-50 rounded-lg px-3 py-2">
          <span class="text-xs text-gray-400 w-16 shrink-0">Κωδικός</span>
          <span class="flex-1 text-sm font-mono text-gray-800 truncate select-none">
            {{ visiblePasswords.has(cred.id) ? cred.password : '••••••••' }}
          </span>
          <button
            @click="togglePassword(cred.id)"
            class="shrink-0 p-1 rounded-md text-gray-400 hover:text-gray-600 transition-colors"
            :title="visiblePasswords.has(cred.id) ? 'Απόκρυψη' : 'Εμφάνιση'"
          >
          ...
          <button
            ...
            :class="copiedKey === cred.id + '-p'
              ...
              : 'bg-white border border-gray-200 text-gray-600 hover:border-blue-300 hover:text-blue-600'"
          >
            {{ copiedKey === cred.id + '-p' ? '✓ Copied' : 'Copy' }}
          </button>
        </div>

        <!-- Open URL -->
        <a v-if="cred.url" :href="cred.url" target="_blank" rel="noopener"
          class="flex items-center gap-1.5 text-xs text-blue-600 hover:text-blue-700 hover:underline mt-1">
    ...
    <div v-if="showModal" class="fixed inset-0 bg-black/40 flex items-end sm:items-center justify-center z-50 px-4 pb-4 sm:pb-0">
      <div class="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
        <h3 class="text-lg font-semibold text-gray-900 mb-5">
          {{ editingId ? 'Επεξεργασία κωδικού' : 'Νέος κωδικός' }}
        </h3>

        <form @submit.prevent="submit" class="space-y-4">
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Όνομα παρόχου / εφαρμογής *</label>
            <input v-model="form.label" type="text" required
              ...
              placeholder="π.χ. ΔΕΗ, ΕΥΔΑΠ, Netflix..." />
          </div>
          ...
          <div>
            ...
            <input v-model="form.password" type="text" autocomplete="new-password"
              class="w-full px-3 py-2.5 rounded-lg border border-gray-300 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-blue-500"
              placeholder="κωδικός πρόσβασης" />
          </div>

          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">URL πληρωμής</label>
