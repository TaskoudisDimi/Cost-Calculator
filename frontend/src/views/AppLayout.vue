    ...
    <nav class="md:hidden fixed bottom-0 inset-x-0 z-30 bg-white border-t border-gray-200"
      ...
      <div class="flex">
        <RouterLink
          ...
          :key="item.name"
          :to="{ name: item.name }"
          class="flex-1 flex flex-col items-center justify-center gap-0.5 py-2.5 transition-colors"
          :class="route.name === item.name ? 'text-blue-600' : 'text-gray-400'"
        >
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
            stroke-width="1.75" stroke="currentColor" class="w-6 h-6"
            v-html="item.icon" />
          <span class="text-[10px] font-medium leading-tight">
            {{ item.name === 'dashboard' ? 'Αρχική' : item.label.split(' ')[0] }}
          </span>
        </RouterLink>
      </div>
    </nav>
