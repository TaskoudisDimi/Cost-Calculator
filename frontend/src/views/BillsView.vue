...
function toggleSelectAll() {
  ...
  else selected.value = new Set(filteredBills.value.map(b => b.id))
}
...
// Feature 5: Greek provider email pattern detection
const GREEK_EMAIL_PATTERNS = [
  ...
  { re: /\bvodafone\.gr\b/i, name: 'Vodafone' },
...
function detectProviderFromEmail(content: string): string | null {
  for (const { re, name } of GREEK_EMAIL_PATTERNS) {
