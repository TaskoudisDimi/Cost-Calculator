...
async function loginGoogle() {
  ...
  try {
    ...
    error.value = t('login.error_google')
  } finally {
...
async function submit() {
  ...
  try {
    ...
    error.value = t('login.error_login')
  } finally {
    loading.value = false
  }
}
...
async function sendReset() {
  ...
  try {
    ...
    resetError.value = t('login.error_no_account')
  } finally {
  ...
  <div class="min-h-screen bg-[#0d1117] flex flex-col items-center justify-center px-4 py-12 relative overflow-hidden">
    ...
    <div class="relative w-full max-w-[360px]">
      ...
      <div class="flex flex-col items-center mb-8">
        ...
        <p class="text-xs text-gray-500 mt-1">{{ t('login.subtitle') }}</p>
      </div>
      ...
      <div class="bg-gray-900 border border-gray-800 rounded-2xl p-7 shadow-2xl">
        ...
        <form @submit.prevent="submit" class="space-y-4">
          <div>
            <label class="block text-xs font-medium text-gray-400 mb-1.5 tracking-wide">{{ t('login.email') }}</label>
            <input
              v-model="email"
              type="email"
              required
          ...
          <div>
            <div class="flex items-center justify-between mb-1.5">
              <label class="text-xs font-medium text-gray-400 tracking-wide">{{ t('login.password') }}</label>
              <button type="button" @click="openReset" class="text-xs text-blue-400 hover:text-blue-300 transition-colors">
                {{ t('login.forgot') }}
              </button>
            </div>
            <input
        ...
        <p class="text-center text-sm text-gray-500 mt-6">
          {{ t('login.no_account') }}
          <RouterLink to="/register" class="text-blue-400 hover:text-blue-300 font-medium transition-colors">{{ t('login.register_link') }}</RouterLink>
        </p>
      </div>
    </div>
    ...
    <div v-if="showReset" class="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 px-4">
      <div class="bg-gray-900 border border-gray-800 rounded-2xl shadow-2xl w-full max-w-sm p-6">
        <div v-if="resetSent" class="text-center py-4">
          ...
          <p class="text-sm font-semibold text-white">{{ t('login.reset_sent_title') }}</p>
          <p class="text-xs text-gray-400 mt-1">{{ t('login.reset_sent_desc') }}</p>
          <button @click="showReset = false"
            class="mt-5 w-full bg-blue-600 hover:bg-blue-500 text-white text-sm font-medium py-2.5 rounded-xl transition-colors">
            {{ t('login.ok') }}
          </button>
        </div>

        <template v-else>
          <h3 class="text-base font-semibold text-white mb-1">{{ t('login.reset_title') }}</h3>
          <p class="text-sm text-gray-400 mb-4">{{ t('login.reset_desc') }}</p>
          <div class="space-y-3">
            <input
              v-model="resetEmail"
              type="email"
            ...
            <div class="flex gap-3">
              ...
              <button @click="sendReset" :disabled="resetLoading || !resetEmail"
                class="flex-1 px-4 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-500 disabled:opacity-55 text-white text-sm font-medium transition-colors">
                {{ resetLoading ? t('login.reset_sending') : t('login.reset_send') }}
              </button>
            </div>
          </div>
        </template>
