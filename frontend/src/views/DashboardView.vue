...
<template>
  <div>
    ...
    <div class="flex items-center justify-between mb-5 gap-3">
      <div>
        <h2 class="text-xl md:text-2xl font-bold text-gray-50">Dashboard</h2>
    ...
    <div v-if="d" class="space-y-4">
      ...
      <div class="bg-gray-800 rounded-xl border border-gray-700 p-4 md:p-5">
        <h3 class="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-4">Μηνιαίος προϋπολογισμός</h3>
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div class="text-center">
            <p class="text-xs text-gray-400 mb-1">Έσοδα</p>
            <p class="text-lg md:text-xl font-bold text-green-400">{{ formatAmount(d.total_income) }}</p>
          </div>
          <div class="text-center">
            <p class="text-xs text-gray-400 mb-1">Λογαριασμοί</p>
            <p class="text-lg md:text-xl font-bold text-gray-100">−{{ formatAmount(d.amount_bills) }}</p>
          </div>
          <div class="text-center">
            <p class="text-xs text-gray-400 mb-1">Αγορές</p>
            <p class="text-lg md:text-xl font-bold text-amber-400">−{{ formatAmount(d.amount_expenses_planned) }}</p>
          </div>
          <div class="text-center md:border-l md:border-gray-100 md:pl-4 pt-2 md:pt-0 border-t border-gray-100 col-span-2 md:col-span-1">
            ...
            <p class="text-xl md:text-xl font-bold" :class="d.remaining >= 0 ? 'text-blue-400' : 'text-red-600'">
      ...
      <div class="bg-gray-800 rounded-xl border border-gray-700 p-4 md:p-5">
        <div class="flex items-center justify-between mb-3">
          ...
          <div class="flex items-center gap-3">
            <button
              ...
              class="text-xs font-medium text-white bg-red-900/200 hover:bg-red-600 px-2.5 py-1.5 rounded-lg transition-colors"
            >
              Διαγραφή ({{ selectedIncomes.size }})
            </button>
            <button @click="showIncomeModal = true" class="text-xs text-blue-400 hover:text-blue-400 font-medium">
              + Προσθήκη
            </button>
          </div>
        </div>
        ...
        <div v-else class="space-y-1">
          ...
          <div class="flex justify-between pt-2 border-t border-gray-100 mt-1">
            <span class="text-sm font-semibold text-gray-300">Σύνολο</span>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <!-- Pending bills -->
        <div class="bg-gray-800 rounded-xl border border-gray-700 p-4 flex flex-col gap-3">
          <div class="flex items-center justify-between">
            <p class="text-xs font-semibold text-gray-400 uppercase tracking-wide">Εκκρεμείς</p>
        ...
        <!-- Overdue bills -->
        <div class="bg-gray-800 rounded-xl border border-red-100 p-4 flex flex-col gap-3">
          <div class="flex items-center justify-between">
            <p class="text-xs font-semibold text-red-400 uppercase tracking-wide">Ληξιπρόθεσμοι</p>
            <span class="text-xs font-bold text-red-400">{{ formatAmount(d.amount_overdue) }}</span>
          </div>
        ...
        <!-- Planned expenses -->
        <div class="bg-gray-800 rounded-xl border border-amber-100 p-4 flex flex-col gap-3">
          <div class="flex items-center justify-between">
        ...
        <div v-else class="space-y-1">
          <div v-for="bill in d.upcoming_bills" :key="bill.id"
            ...
            <div class="flex items-center gap-2 shrink-0">
              <p class="text-xs hidden sm:block" :class="bill.status === 'overdue' ? 'text-red-400' : 'text-gray-400'">
                {{ formatDate(bill.due_date) }}
              </p>
              <p class="text-sm font-semibold text-gray-50">{{ formatAmount(bill.amount) }}</p>
